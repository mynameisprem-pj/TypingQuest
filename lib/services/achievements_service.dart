...
// O(1) lookup map — built once at startup from the list above.
final Map<String, Achievement> _achievementMap = {
...
class AchievementsService {
  ...
  /// Fires whenever a new achievement is unlocked.
  ...
  bool isUnlocked(String id) =>
      _prefs?.getBool('${_prefix}ach_$id') ?? false;

  Future<bool> unlock(String id) async {
    if (isUnlocked(id)) return false;
    await _prefs?.setBool('${_prefix}ach_$id', true);
    final ach = _achievementMap[id]; // O(1) — was O(n) linear search
  ...
  int get totalUnlocked => allAchievements.where((a) => isUnlocked(a.id)).length;
  int get total         => allAchievements.length;

  // ── Bulk check & auto-unlock ──────────────────────────────────────────

  /// Call after any game session with the stats that may have changed.
  ///
  /// [accuracyStreak] — number of consecutive sessions above 90 % accuracy.
  /// Pass this so the [acc_streak_5] achievement can be checked. Previously
  /// this parameter was missing and the achievement could never unlock.
  Future<void> checkAll({
    int?    bestWpm,
    double? lastAccuracy,
    int?    accuracyStreak,   // consecutive sessions >= 90 % accuracy
    int?    totalSessions,
    int?    totalWords,
    ...
    if ((accuracyStreak ?? 0) >= 5) await unlock('acc_streak_5');
