...
const List<String> _kWords = [
  ...
  'any', 'same', 'tell', 'boy', 'following', 'came', 'want', 'show',
  'also', 'around', 'form', 'small', 'set', 'put', 'end', 'does',
  'another', 'well', 'large', 'need', 'big', 'high', 'such', 'turn',
  'here', 'why', 'went', 'men', 'read', 'land', 'different', 'home',
  ...
  'should', 'world', 'school', 'keep', 'plant', 'cover', 'food', 'sun',
  'computer', 'type', 'fast', 'practice', 'keyboard', 'finger', 'speed',
  'level', 'Nepal', 'mountain', 'student', 'teacher', 'class', 'book',
];

// Cached Random instance — no need to create a new one per call.
final _rng = Random();

String _generateText(int targetWords) {
  final words = List.generate(
    targetWords,
    (_) => _kWords[_rng.nextInt(_kWords.length)],
  );
  return words.join(' ');
}
...
class _TimedChallengeScreenState extends State<TimedChallengeScreen>
  ...
  void initState() {
    ...
    // easeOutCubic: smooth, no spring-physics overhead on every frame.
    // Curves.elasticOut recalculates spring forces on every tick — unnecessary
    // for a result card that is shown once.
  ...
  Future<void> _pickCustomTime() async {
    final minutes = _isCustom ? _selectedDuration ~/ 60 : 3;
    final seconds = _isCustom ? _selectedDuration % 60  : 0;
  ...
  void _handleKey(KeyEvent event) {
    ...
    if (char == expected) {
      ...
      // Generate more text when running low.
  ...
  // Returns the correct timer colour directly — previously returned a String
  // that was then converted to a Color in _buildTypingView (dead indirection).
  Color get _timerColor {
    if (_timeLeft > 30) return AppTheme.primary;
    // Both 'warning' and 'danger' previously mapped to AppTheme.error — unified.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: KeyboardListener(
        ...
        child: _finished
            ? _buildResults(accuracy)
            : _buildTypingView(accuracy),
      ),
    );
  }

  Widget _buildTypingView(double accuracy) {
    final tColor = _timerColor; // single call — no repeated getter evaluation
            ...
            children: [
                      ...
                      child: GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            color: _selectedDuration == _durations[e.key] &&
                                    !_isCustom
              ...
              Text(
                _timeLeft >= 60
                    ...
                    : '$_timeLeft',
                style: AppTheme.heading(28, color: tColor),
              ),
              const SizedBox(width: 24),
              _StatsChip('WPM', '$_wpm', AppTheme.primary),
              const SizedBox(width: 16),
              _StatsChip('ACC', '${accuracy.toStringAsFixed(0)}%',
                  AppTheme.gold),
            ],
          ),
        ),
  ...
  Widget _buildTextArea() {
    // ── Sliding window ────────────────────────────────────────────────────
    // Previously rendered from character 0 to _currentIndex + 200, meaning
    // the completed-char list grew without bound (1 000 – 7 000 widgets in a
    // Wrap for a long session). Now we keep at most 100 chars behind the
    // cursor and 250 ahead — a constant-size window that doesn't grow.
    final windowStart = max(0, _currentIndex - 100);
    final windowEnd   = min(_currentIndex + 250, _text.length);
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Wrap(
          children: List.generate(windowEnd - windowStart, (wi) {
            final i     = windowStart + wi;
            Color color;
            Color? bg;
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(
                      child: _ResultStat(
                          'WPM', '$_wpm', AppTheme.primary)),
                  Container(
                      width: 1, height: 50, color: AppTheme.cardBorder),
                  Expanded(
                      child: _ResultStat(
                          'ACCURACY',
                  ...
                  Expanded(
                      child: _ResultStat(
                          'WORDS', '$wordsTyped', AppTheme.success)),
                ],
              ),
              const SizedBox(height: 24),
              Container(
                ...
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      ...
                      onPressed: () => _reset(_selectedDuration,
                          isCustom: _isCustom),
                    ),
                  ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Set your own practice duration.',
                style: AppTheme.body(13, color: AppTheme.textSecondary)),
            const SizedBox(height: 24),
            _PickerRow(
                label: 'Minutes', value: _minutes, min: 0, max: 99,
                onChanged: (v) => setState(() => _minutes = v)),
            const SizedBox(height: 16),
            _PickerRow(
                label: 'Seconds', value: _seconds, min: 0, max: 59,
                step: 5,
