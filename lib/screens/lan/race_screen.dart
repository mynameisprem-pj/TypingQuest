...
class RaceScreen extends StatefulWidget {
  ...
  final int? startAtMs;
  final List<LanPlayer> initialPlayers; // <── pass from waiting room
...
class _RaceScreenState extends State<RaceScreen> {
  ...
  bool _allDone = false; // set true when server sends race_complete
  ...
  void initState() {
    ...
    // Seed with initial player list from waiting room so standings show immediately
    _allPlayers = List.from(widget.initialPlayers);
    _remainingSecs = widget.timeLimitSeconds;
    _serverStartAtMs = widget.startAtMs;
    _phase = _serverStartAtMs == null ? LanRacePhase.racing : LanRacePhase.countdown;

    _stopwatch = Stopwatch();
    if (_phase == LanRacePhase.racing) {
      _stopwatch.start();
    }
    ...
    _uiTimer = Timer.periodic(const Duration(milliseconds: 200), (_) {
      ...
      setState(() {
        _elapsedSecs = _stopwatch.elapsed.inSeconds;
        _updateWpm();
        if (widget.timedMode && _phase == LanRacePhase.racing) {
          _remainingSecs = (widget.timeLimitSeconds - _elapsedSecs)
              .clamp(0, widget.timeLimitSeconds);
          if (_remainingSecs == 0 && !_timedEnded) _onTimedEnd();
        }
      });
    });
  ...
  void _updateWpm() {
    if (_elapsedSecs > 0) {
      _wpm = ((_currentIndex / 5) / (_elapsedSecs / 60)).round();
    }
  ...
  void _onTimedEnd() {
    ...
    _uiTimer?.cancel();
    _progressTimer?.cancel();
    _updateWpm(); // calculate final stats first
    _sendProgress(force: true); // send before setting _timedEnded so it isn't blocked
    _timedEnded = true;
    _finished = true;
    if (mounted) setState(() {});
  }
  ...
  void _onFinish() {
    ...
    _progressTimer?.cancel();
    _updateWpm();
    _sendProgress(force: true); // send final stats before setting _finished
    _finished = true;
  ...
  Widget _buildStatusBanner(double progress) {
    ...
    if (_timedEnded) {
      return Container(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Time\'s up!', style: AppTheme.heading(15, color: AppTheme.success)),
    ...
    // I finished, waiting for others (race mode)
      ...
      return Container(
        ...
        child: Row(children: [
          ...
          Text('$_remainingSecs seconds left!',
              style: AppTheme.body(14, color: AppTheme.error, weight: FontWeight.bold)),
        ]),
      );
    }
  ...
  Widget _buildProgressRow(double progress) {
    final timerProgress = (_elapsedSecs / widget.timeLimitSeconds).clamp(0.0, 1.0);
    return Row(children: [
      Text(widget.playerName, style: AppTheme.body(13, color: AppTheme.primary)),
      const SizedBox(width: 10),
      ...
      const SizedBox(width: 10),
      Text(
        widget.timedMode ? '$_wordsTyped words' : '${(progress * 100).round()}%',
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: _lastWasWrong
              ...
              : _finished ? AppTheme.success.withValues(alpha: 0.3) : AppTheme.cardBorder,
  ...
  // Timed mode results in text box
  Widget _buildTimedResults() {
    final sorted = _sortedPlayers;
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Text('⏱', style: TextStyle(fontSize: 40)),
      const SizedBox(height: 10),
      Text('Time\'s Up!', style: AppTheme.heading(24, color: AppTheme.gold)),
      ...
      if (sorted.length > 1) ...[
        ...
        ...sorted.asMap().entries.map((e) {
          ...
          return Container(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
            decoration: BoxDecoration(
              color: isMe ? AppTheme.primary.withValues(alpha: 0.08) : AppTheme.surface,
              borderRadius: BorderRadius.circular(8),
              border: Border.all(
                  color: isMe ? AppTheme.primary.withValues(alpha: 0.3) : AppTheme.cardBorder),
            ),
            child: Row(children: [
              Text(i == 0 ? '🥇' : i == 1 ? '🥈' : '🥉',
  ...
  // Race mode results — shown when ALL players finished
  Widget _buildRaceResults() {
    if (!_allDone) {
      // Still waiting for others — just show "you finished" in the box too
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        const Text('🏁', style: TextStyle(fontSize: 44)),
        const SizedBox(height: 12),
        Text('You finished!', style: AppTheme.heading(22, color: AppTheme.success)),
    ...
    // Everyone finished — show podium
    final sorted = _sortedPlayers;
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Text('🏆', style: TextStyle(fontSize: 44)),
      const SizedBox(height: 10),
      Text('Race Complete!', style: AppTheme.heading(24, color: AppTheme.gold)),
      const SizedBox(height: 16),
      ...sorted.asMap().entries.map((e) {
        final i = e.key; final p = e.value;
        final isMe = p.name == widget.playerName;
        return Container(
          ...
          decoration: BoxDecoration(
            color: i == 0
                ...
                : isMe ? AppTheme.primary.withValues(alpha: 0.07) : AppTheme.surface,
            borderRadius: BorderRadius.circular(8),
            border: Border.all(
              color: i == 0
                  ? AppTheme.gold.withValues(alpha: 0.4)
                  : isMe ? AppTheme.primary.withValues(alpha: 0.3) : AppTheme.cardBorder,
            ),
          ),
          child: Row(children: [
            Text(i == 0 ? '🥇' : i == 1 ? '🥈' : '🥉',
            ...
            Expanded(child: Text(p.name + (isMe ? ' (You)' : ''),
                style: AppTheme.body(14,
                    color: i == 0 ? AppTheme.gold
                        : isMe ? AppTheme.primary : AppTheme.textPrimary,
                    weight: i == 0 ? FontWeight.bold : FontWeight.normal))),
            Text('#${p.rank ?? i + 1}  •  ${p.wpm} wpm',
                style: AppTheme.body(12,
                    color: i == 0 ? AppTheme.gold : AppTheme.textSecondary)),
          ]),
        );
      }),
    ]));
          ...
          Expanded(child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  color: isMe ? AppTheme.primary.withValues(alpha: 0.08) : AppTheme.card,
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(
                    color: isMe
                        ? AppTheme.primary.withValues(alpha: 0.3) : AppTheme.cardBorder),
                ),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Text('${i + 1}.',
                    ...
                    Text(
                      widget.timedMode ? '${p.wordsTyped}w' : '${p.wpm}wpm',
                      style: AppTheme.body(10, color: AppTheme.textSecondary),
                    ),
                  ]),
                  ...
                  ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      ...
                      valueColor: AlwaysStoppedAnimation(
                        p.finished
                            ? AppTheme.success
                            : isMe ? AppTheme.primary : AppTheme.gold,
                      ),
                      minHeight: 5,
                    ),
                  ),
                  if (p.finished && !widget.timedMode)
                    Padding(
                      ...
                      child: Row(children: [
                        const Icon(Icons.flag, color: AppTheme.success, size: 11),
                        const SizedBox(width: 4),
                        Text('Finished! #${p.rank ?? '?'}',
                            style: AppTheme.body(10, color: AppTheme.success)),
                      ]),
                    ),
                  if (p.dnf || p.disconnected)
                    Padding(
                      padding: const EdgeInsets.only(top: 4),
                      child: Row(children: [
                        const Icon(Icons.link_off, color: AppTheme.error, size: 11),
                        const SizedBox(width: 4),
                        Text('Disconnected / DNF',
                            style: AppTheme.body(10, color: AppTheme.error)),
                      ]),
                    ),
                ]),
              );
...
class _ModeBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      color: timed ? AppTheme.gold.withValues(alpha: 0.15) : AppTheme.primary.withValues(alpha: 0.12),
      borderRadius: BorderRadius.circular(12),
    ),
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      Icon(timed ? Icons.timer_rounded : Icons.flag_rounded,
          color: timed ? AppTheme.gold : AppTheme.primary, size: 12),
      const SizedBox(width: 4),
      Text(timed ? 'TIMED' : 'RACE',
